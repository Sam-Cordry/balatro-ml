...
use crate::{
    ...
    model::{
        ...
        db::{CardRow, ConsumableRow, ConsumableType, JokerRow, JokerType, SessionRow},
...
pub async fn get_game_state(path: Path<i32>, state: Data<AppState>) -> impl Responder {
    ...
    let consumables = match sqlx::query_as!(
        ConsumableRow,
        r#"SELECT session, consumable as "consumable: ConsumableType", negative FROM consumables WHERE session = $1"#,
        session_id
    ).fetch_all(&state.db).await {
        Ok(data) => data,
